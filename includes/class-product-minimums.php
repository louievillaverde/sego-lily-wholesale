...
class SLW_Product_Minimums {
	...
	public static function init() {
		...
		// Product-type minimums: "6 per Ageless, 6 per Renewal" (Holly, 2026-04-29
		// call). Enforced as a per-PRODUCT total summed across that product's
		// variations, so scents mix to hit it. Category minimums (Gift Sets = 4)
		// run alongside for the cross-product case.
	 ...
	 * product's variations. Holly's rule (2026-04-29 call): "six per Ageless,
	 * six per Renewal" met by mixing that line's scents (3 honey + 3 lavender,
	 * or 2/2/2), never forced to buy 6 of one scent. Only products the customer
	 * has actually added are flagged. Structured entries match
	 * SLW_Category_Minimums so the order-form panel renders product + category
	 * identically. (Gift Sets stay a category minimum: 4 across gift-set types.)
	 */
	...
	/**
	 * Enforce product-type minimums at cart validation: an error notice per
	 * product under its minimum blocks checkout. Same source as the order-form
	 * panel so the two surfaces always agree.
	 */
