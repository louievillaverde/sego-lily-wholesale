...
add_action( 'plugins_loaded', function() {
    ...
    require_once SLW_PLUGIN_DIR . 'includes/class-scent-library.php';
    ...
    require_once SLW_PLUGIN_DIR . 'includes/class-wholesale-checkout.php';
    ...
    require_once SLW_PLUGIN_DIR . 'includes/class-scent-availability.php';
    ...
    require_once SLW_PLUGIN_DIR . 'includes/class-admin-new-order.php';
    require_once SLW_PLUGIN_DIR . 'includes/class-xero-sync.php';
    require_once SLW_PLUGIN_DIR . 'includes/class-rest-stats.php';
    require_once SLW_PLUGIN_DIR . 'includes/class-packing-slip.php';
    ...
    SLW_Scent_Availability::init();
    SLW_Customers_Page::init();
    SLW_Shipment_Tracking::init();
    SLW_Admin_New_Order::init();
    SLW_Xero_Sync::init();
    SLW_Rest_Stats::init();
    SLW_Packing_Slip::init();
    ...
    add_action( 'wp_enqueue_scripts', function() {
        if ( is_page() || has_shortcode( get_post()->post_content ?? '', 'sego_wholesale_application' )
            ...
            || has_shortcode( get_post()->post_content ?? '', 'sego_wholesale_checkout' )
...
register_activation_hook( __FILE__, function() {
    ...
    $pages = array(
        ...
        // Wholesale-only checkout page. Uses the CUSTOM SLW shortcode that
        // builds its own form + computes line totals from our wholesale
        // helper, then creates a real WC order via wc_create_order() so
        // the order still feeds WC analytics, reports, customer history.
        // Wholesale users get redirected here from /checkout via
        // SLW_Nav_Menu::redirect_checkout_for_wholesale.
        'wholesale-checkout' => array(
            'title'   => 'Wholesale Checkout',
            'content' => '[sego_wholesale_checkout]',
        ),
        'quiz-results' => array(
